 ...
 * -------- -------- ---------------------------------- ------------------------------------------------------
 * 12/9/09  jmaxxz initial development                Fully implemented every method
 * 2/8/09   jmaxxz Bug 278                            Added support for having no listeners
 */
...
namespace Ares.SharperGps
{
    ...
    public class GpsHandler : IDisposable
    {
        ...
        public void Start(string serialPort, int baudRate)
        {
            ...
            _clThread = new Thread(_clThreadStart) {IsBackground = true};
            _clThread.Start();
